...
namespace VK4 {
	...
	public:
		...
		~Vk_Instance() {
			Vk_Logger::Log(typeid(this), GlobalCasters::castDestructorTitle(std::string("Destroy Instance: ") + _applicationName));
#ifdef _DEBUG
			if (_debugMessenger) {
	...
	private:
		...
		void initApplication() {
			...
			for (uint32_t i = 0; i < static_cast<uint32_t>(requiredValidationLayers.size()); ++i) {
				...
				if (!found) {
					Vk_Logger::RuntimeError(typeid(this), "Required validation layer is missing: {0}", requiredValidationLayers[i]);
					break;
				}
			}
#endif
			...
			ASSERT_DEBUG_MSG(func, "Failed to create debug messanger!");
		...
		static VKAPI_ATTR VkBool32 VKAPI_CALL _debugCallback(
			...
			void* pUserData)
		{
			std::string message = GlobalCasters::castValicationLayer(pCallbackData->pMessage);
			switch (messageSeverity) {
			case VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT:
				Vk_Logger::Error(typeid(NoneObj()), message); // red
				break;
			case VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT:
				Vk_Logger::Warn(typeid(NoneObj()), message); // yellow
				break;
			case VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT:
				Vk_Logger::Log(typeid(NoneObj()), message); // white or other
				break;
			case VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT:
				Vk_Logger::Trace(typeid(NoneObj()), message); // white or other
				break;
			default:
				break;
			}
