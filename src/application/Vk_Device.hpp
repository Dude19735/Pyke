...
namespace VK4 {
	...
	class  Vk_Device {
		typedef int current_frame_index;

		struct Bridge {
			int currentFrame;
			std::map<current_frame_index, std::queue<std::function<void()>>> updates;
		};
...
private:
	...
	public:
		...
		Vk_Device(std::string deviceName, Vk_DevicePreference devicePreferences = Vk_DevicePreference::USE_ANY_GPU)
			:
			bridge(Bridge{.currentFrame=0}),
			_viewer(0),
			_device(nullptr),
			_graphicsQueues({}),
			_presentationQueues({}),
		...
		const QueueFamilyIndex* vk_queueFamilyIndices() const {
		...
		SingleTimeCommand vk_beginSingleTimeCommands(CommandCapabilities command) {
		...
		void vk_endSingleTimeCommands(const SingleTimeCommand& singleTimeCommand) {
	...
	private:
		...
		PhysicalDevice* _activePhysicalDevice;
		int _physicalDeviceIndex;
		std::unique_ptr<Vk_Instance> _instance;
		std::unordered_map<PhysicalDevice*, std::map<VkMemoryPropertyFlags, GpuMemoryConfiguration>> _gpuMemoryConfig;
		std::unordered_map<PhysicalDevice*, std::vector<GpuMemoryHeapConfiguration>> _gpuHeapConfig;

		std::vector<SingleTimeCommand> _singleTimeCommandBuffer;

		Vk_DevicePreference _devicePreferences; 
		...
		void createCopyToBufferCommand(
		...
		VkBool32 getSupportedDepthFormat(VkFormat& depthFormat, PhysicalDevice* pDev)
		{
		...
		const SwapchainSupportDetails& vk_swapchainSupport(PhysicalDevice* pDev, const Vk_Surface* surface) {
		...
		bool _setPhysicalDevice(Vk_Surface* surface)
		{
			...
			std::vector<PhysicalDevice>::iterator pDev = _physicalDevices.begin();
			int index = 0;
		...
		bool findGpuForDevicePreferences(const PhysicalDevice& physicalDevice){
			if(_devicePreferences == Vk_DevicePreference::USE_ANY_GPU){
				return (
			...
			if(_devicePreferences == Vk_DevicePreference::USE_DISCRETE_GPU){
				return (
					(physicalDevice.deviceProperties.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU)
				);
			}

			if(_devicePreferences == Vk_DevicePreference::USE_INTEGRATED_GPU){
				return ( 
					(physicalDevice.deviceProperties.deviceType == VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU)
				);
			}
		...
		void setGpuMemoryConfig() {
			for (auto& device : _physicalDevices) {
				...
				for (uint32_t i = 0; i < deviceMemoryProperties.memoryHeapCount; ++i) {
					...
					double s_kb = round(s / 1.0e3, 2);
					double s_mb = round(s / 1.0e6, 2);
					double s_gb = round(s / 1.0e9, 2);
		...
		void devicePropertyFlagToString(const VkMemoryPropertyFlags flags, std::string& propStrShort, std::string& propStrLong) {
			...
			while (replace(type, "VK_MEMORY_PROPERTY_", "")) {};
			while (replace(type, "_BIT", "")) {};
			while (replace(type, "_", " ")) {};
		...
		std::string rightCrop(double value) {
