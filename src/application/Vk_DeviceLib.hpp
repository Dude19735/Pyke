		...
		*/
        ...
        static SwapchainSupportDetails swapchainSupport(PhysicalDevice* pDev, const Vk_Surface* surface, bool multiImageBuffering, Bridge& bridge) {
			...
			if (!getSupportedDepthFormat(swapchainSupportDetails.depthFormat, pDev)) {
				Vk_Logger::RuntimeError(typeid(NoneObj), GlobalCasters::castHighlightRed("No supported depth formats found!"));
			}
			...
			if (multiImageBuffering) {
				...
				if (caps.maxImageCount > 0 && swapchainSupportDetails.nFramesInFlight > caps.maxImageCount) {
					Vk_Logger::RuntimeError(typeid(NoneObj), "Double Buffering not supported by GPU. Max supported Frames-In_Flight are " + caps.maxImageCount);
				...
				if(caps.minImageCount > 1){
					Vk_Logger::Warn(typeid(NoneObj), "Single image not supported by swapchain. Minimum " + std::to_string(caps.minImageCount) + " images required! Setting nFramesInFlight to "  + std::to_string(caps.minImageCount) + ".");
				}
			}
        ...
        static bool setPhysicalDevice(
			...
			if (deviceCount == 0)
				Vk_Logger::RuntimeError(typeid(NoneObj), GlobalCasters::castHighlightRed("Failed to find devices with Vulkan support!"));
			...
			for (const auto& physicalDevice : vkPhysicalDevices) {
				...
				if (!physicalDevices[i].queueFamilyIndices.isComplete()) {
					Vk_Logger::RuntimeError(typeid(NoneObj), "No suitable graphics device found");
				}
        ...
        static uint32_t findMemoryType(
			...
			VK4::Vk_Logger::RuntimeError(typeid(NoneObj), "Failed to find suitable memory type!");
        ...
        static void createImage(
			...
			if (vkAllocateMemory(device, &allocInfo, nullptr, &imageMemory) != VK_SUCCESS) {
				Vk_Logger::RuntimeError(typeid(NoneObj), "failed to allocate image memory!");
			}
		...
		static void endSingleTimeCommands(
			...
			if(res != VK_SUCCESS){
				if(res == VK_ERROR_OUT_OF_HOST_MEMORY){
					...
					Vk_Logger::RuntimeError(typeid(NoneObj), "vkQueueSubmit failed with VK_ERROR_OUT_OF_DEVICE_MEMORY ({0})!", static_cast<int64_t>(res));
				}
				else{
					Vk_Logger::RuntimeError(typeid(NoneObj), "vkQueueSubmit failed with {0}!", static_cast<int64_t>(res));
				}
			}
        ...
        static void transitionImageLayout(
			...
			if (oldLayout == VK_IMAGE_LAYOUT_UNDEFINED && newLayout == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL) {
				...
				VK4::Vk_Logger::RuntimeError(typeid(NoneObj), "Unsupported layout transition!");
			}
		...
		static void copyBuffer(
			...
			if(size == 0){
				Vk_Logger::Error(typeid(NoneObj), "Attempted to copy 0 bytes to device buffer {0}", associatedObject);
				return;
			}
        ...
        static uint32_t getMemoryTypeIndex(
            ...
            if(activePhysicalDevice == nullptr){
                Vk_Logger::RuntimeError(typeid(NoneObj), "No active physical device is set. Run setPhysicalDevice first!");
            }
        ...
        static VkResult createBuffer(
			...
			if (res != VK_SUCCESS) {
				if (res == VK_ERROR_OUT_OF_DEVICE_MEMORY) {
					...
					Vk_Logger::RuntimeError(typeid(NoneObj), "failed to allocate buffer memory!");
				}
			}
