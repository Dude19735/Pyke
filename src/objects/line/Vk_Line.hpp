...
namespace VK4 {
	...
	namespace Vk_Line_Structures {
		typedef struct Vk_PushConstants {
			float alpha;
		} Vk_PushConstants;
	}

	static std::string lineObjectTypes() {
		std::string tab = "\t";
		std::string nl = "\n";
		return 
	...
	template<class T>
	class Vk_Line : I_Object<T> {
	public:

		Vk_Line() : I_Object<ObjectType_Info>(nullptr, "None", "None", "Vk_Line<T>", VK4::Topology::Lines, VK4::CullMode::NoCulling, VK4::RenderType::Wireframe) {
			Vk_Logger::RuntimeError(typeid(this), 
				GlobalCasters::castRed("Attempt to instantiate Vk_Line with invalid template format ") + 
	...
	template<> class Vk_Line<ObjectType_Info> : I_Object<ObjectType_Info> {
	public:
		Vk_Line() : I_Object<ObjectType_Info>(nullptr, "None", "None", glm::tmat3x3<point_type>(), "Vk_Line<ObjectType_Info>", VK4::Topology::Lines, VK4::CullMode::NoCulling, VK4::RenderType::Wireframe) {
	...
	template<> 
	class Vk_Line<ObjectType_P_C> : public I_Object<ObjectType_P_C> {
	public:
...
// ############################################################################################################
		...
		void vk_30_bindPushConstants(const Vk_BindingProperties& props) override {
			...
			Vk_Line_Structures::Vk_PushConstants constants = {};
			...
			vkCmdPushConstants(
				...
				0,
				sizeof(Vk_Line_Structures::Vk_PushConstants),
				&constants
			);
		}
		...
		void vk_40_bindUniformBuffers(const Vk_BindingProperties& props) override {
			...
			if(props.uniformBuffers.find(1) != props.uniformBuffers.end()){
				Vk_Logger::RuntimeError(typeid(this), "Vk_Line requires binding at 1 but it's already taken");
			}
